...
export const ClientMQTTProvider = ({
	children,
	userData,
	onUserDataUpdate,
}) => {
	...
	useEffect(() => {
		...
		if (!userData || !userData.id || userData.is_admin === true) {
			console.log(
				'🚫 Клиентский MQTT отключен - пользователь админ или данные неполные'
			)
		...
		client.on('connect', () => {
			...
			client.subscribe(personalTopicName, { qos: 1 }, err => {
				if (err) {
					...
					console.log(
						'👤 Подписка на персональный топик установлена:',
						personalTopicName
					)
				}
			})
		...
		client.on('message', (topic, message) => {
			try {
				...
				setMessages(prev => [
					...prev,
					{ ...parsedMessage, topic, timestamp: Date.now() },
				])
				...
				if (
					topic.includes('_confirmation') &&
					parsedMessage.type === 'user_confirmation'
				) {
					...
					if (userData && parsedMessage.user_id === userData.id) {
						...
						if (onUserDataUpdate) {
							onUserDataUpdate({
								...userData,
								confirmed: parsedMessage.confirmed,
							})
						}
					}
				}
	...
	const subscribeToUserScheduleZones = async mqttClient => {
		try {
			...
			if (
				...
				const todaySchedules = result.schedules.filter(
					schedule => schedule.date.startsWith(today) // Проверяем, что дата начинается сегодняшней даты (schedule.date в формате "YYYY-MM-DDTHH:mm:ss.sssZ")
				)

				if (todaySchedules.length > 0) {
					// Извлекаем уникальные ID зон из сегодняшнего расписания
					const zoneIds = [
						...new Set(todaySchedules.map(schedule => schedule.zone_id)),
					]

					console.log(
						`📅 Найдены зоны из сегодняшнего расписания пользователя: ${zoneIds.join(
							', '
						)}`
					)
					...
					zoneIds.forEach(zoneId => {
						...
						mqttClient.subscribe(zoneTopic, { qos: 1 }, err => {
							if (err) {
								console.error(
									`❌ Ошибка подписки на топик зоны ${zoneId}:`,
									err
								)
							} else {
								console.log(
									`📍 Подписка на топик зоны ${zoneId} установлена (из сегодняшнего расписания)`
								)
							}
						})
					})
					...
					console.log(
						`✅ Клиент подписан на ${zoneIds.length} зон из сегодняшнего расписания`
					)
				} else {
					...
					console.log(
						`🔄 Используем резервные зоны: ${defaultZoneIds.join(', ')}`
					)
					...
					defaultZoneIds.forEach(zoneId => {
						...
						mqttClient.subscribe(zoneTopic, { qos: 1 }, err => {
							if (err) {
								...
								console.log(
									`📍 Подписка на топик зоны ${zoneId} установлена (резервная)`
								)
							}
						})
					})
				...
				console.log(
					'📅 У пользователя нет расписания на текущую неделю или ошибка API'
				)
				// Подписываемся на возможные зоны по умолчанию
				const defaultZoneIds = [1, 2, 3, 4, 5]
				console.log(
			...
			console.error(
				'❌ Ошибка получения зон из расписания пользователя:',
				error
			)
			// В случае ошибки подписываемся на диапазон зон как fallback
			console.log('🔄 Используем fallback подписку на зоны 1-5')
